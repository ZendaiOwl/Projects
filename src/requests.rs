...
impl ApiReq<'_> {
    ...
    pub fn create_request(method: &str, url: URL, req_body: ReqBody) -> Req {
        Request::builder()
            .method(method)
            .uri(url)
            .header("Content-Type", "application/json")
    ...
    pub fn create_respone(content: &str, status: u16, body: ReqBody) -> Resp {
        Response::builder()
    ...
    pub fn create_image(method: &str, url: URL, req_body: ReqBody, base64_json: String) -> Req {
        Request::builder()
            .method(method)
            .uri(url)
    ...
    pub fn image_from_container(method: &str, url: URL, req_body: ReqBody) -> Req {
        Request::builder()
            .method(method)
            .uri(url)
            .header("Content-Type", "application/json")
    ...
    pub fn build_request(method: &str, url: URL, req_body: ReqBody, base64_json: String) -> Req {
        Request::builder()
            .method(method)
            .uri(url)
    ...
    pub fn json_to_response(jsbody: JSON, code: StatusCode) -> Resp {
    ...
    pub fn json_to_body(jsbody: JSON) -> ReqBody {
        Body::from(jsbody.to_string())
    }

    pub fn rbody(r: String) -> ReqBody {
        Body::from(r)
    }
    
    pub async fn res_to_string(mut r: Resp) -> String {
    ...
    pub async fn res_to_json(mut r: Resp) -> JSON {
        let mut response_data = Vec::new();
        while let Some(next) = r.data().await {
            let chunk = next.expect("Unable to parse response data");
            response_data.extend_from_slice(&chunk);
    ...
    pub async fn res_to_img_list(mut r: Resp) -> ListImages {
        let mut response_data = Vec::new();
        while let Some(next) = r.data().await {
            let chunk = next.expect("Unable to parse response data");
            response_data.extend_from_slice(&chunk);
        }
        let msg: ListImages = serde_json::from_slice(&response_data).expect("Failed to parse JSON from slice");
        msg
    }

    pub async fn base64_to_json(r: Resp) -> JSON {
        let header = r
            .headers()
    ...
    pub async fn into_writer(mut r: Resp, mut writer: impl std::io::Write) {
        while let Some(next) = r.data().await {
            let chunk = next.expect("Unable to parse response data");
            writer
                .write_all(&chunk)
    ...
    pub async fn into_json_writer(json: JSON, writer: impl std::io::Write) {
