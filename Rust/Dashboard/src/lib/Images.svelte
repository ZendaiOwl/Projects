<script>
    ...
    async function fetch_image() {
        let image_to_pull;
        if (tag_name != null) {
            image_to_pull = image_name.toLowerCase() + ":" + tag_name.toLowerCase();
        } else {
            image_to_pull = image_name.toLowerCase();
        };
    ...
    async function delete_image(ID) {
        await invoke('delete_image', {
            id: ID.Id.substring(7,18), force: false, noprune: false
        }).then((result) => {
            update_image_list();
        }).catch((error) => {
            dialog_data = error;
      ...
      <div class="box">
        <div class="table-container">
          ...
          <table class="table is-narrow is-striped is-hoverable sortable" 
            ...
            <tbody>
              {#each images_list as { Containers, RepoTags, Created, Size, Id } }
              <tr>
                <td>
                ...
                <td>{strip_container_name(RepoTags)}</td>
                <td>{strip_container_tag(RepoTags)}</td>
