<script>
  ...
  async function stop_container(ID) {
    await invoke('container_stop', {
      id: ID.Id, signal: "SIGTERM", time: 3
    }).then((result) => {
      update_containers_list();
    }).catch((error) => {
      dialog_data = error;
  ...
  async function handle_creation(event) {
    ...
    if (container_port_value != undefined) {
      obj.HostConfig.PortBindings = {
        [port_binding_host]: [
          {
            HostPort: container_port_value.toString()
          }
        ]
      };
    };
    ...
    // obj.NetworkConfig = {};
    // obj.NetworkConfig.EndpointsConfig = {};
    // obj.NetworkConfig.EndpointsConfig.isolated_nw = {};
    // obj.NetworkConfig.EndpointsConfig.isolated_nw.IPAMConfig = {};
    // obj.NetworkConfig.EndpointsConfig.isolated_nw.Links = {};
    // obj.NetworkConfig.EndpointsConfig.isolated_nw.Aliases = {};
    
    if (invalid == false) {
      if (data.Name != '') {
        await invoke('container_create_with_name', {
          name: data.Name, request: obj
        }).then((result) => {
            ...
            <div class="control is-expanded">
              <div class="select">
                <select name="Image" bind:value={image} id="image_select" required>
                  ...
                  {#each images_list as { Names } }
                    <option>{strip_name(Names)}</option>
                  {/each}
                </select>
              </div>
            </div>
          ...
          <table class="table is-narrow is-striped is-hoverable" s
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="has-text-centered"><div class="content" style="width: 100px;">{Status}</div></td>
