        ...
        for nodes in self.copy() {
            for node in nodes {
                if node.is_dir() {
                    ...
                    Format::Dir(&node.strip_prefix(&pfx).expect("No such prefix").display().to_string()).print();
                } else {
                    // println!("{}", node.strip_prefix(&pfx).expect("No such prefix").display());
                    Format::File(&node.strip_prefix(&pfx).expect("No such prefix").display().to_string()).print();
                }
            }
        }
    }
